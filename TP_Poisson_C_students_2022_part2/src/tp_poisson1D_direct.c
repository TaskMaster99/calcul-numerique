...
#define IDP 3 /* Set identity Column Major Band*/
...
/* ** argv: Valeur des arguments */
{
  ...
  if (IMPLEM == TRF) {
    ...
    write_GB_operator_colMajor_poisson1D(AB, &lab, &la, "LACKLU.dat");
  }
  ...
  if(IMPLEM == IDP)
  {
    /* Allocate and initialize ID matrix*/
    double* IDM = (double *) malloc(sizeof(double)*lab*la);
    set_GB_operator_colMajor_poisson1D_Id(IDM, &lab, &la, &kv);
    write_GB_operator_colMajor_poisson1D(IDM, &lab, &la, "IDM.dat");
    free(IDM);
  }
